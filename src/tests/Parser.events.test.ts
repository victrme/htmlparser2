...
import * as helper from './_helper.ts'
...
describe('Events', () => {
	...
	it('Template script tags', () => runTest('<p><script type="text/template"><h1>Heading1</h1></script></p>'))

	it('Lowercase tags', () => runTest('<H1 class=test>adsf</H1>', { lowerCaseTags: true }))
	...
	it('CDATA (inside special)', () =>
		runTest('<script>/*<![CDATA[*/ asdf ><asdf></adsf><> fo/*]]>*/</script>'))

	it('leading lt', () => runTest('>a>'))

	it('end slash: void element ending with />', () => runTest('<hr / ><p>Hold the line.'))
	...
	it('end slash: non-void element ending with />', () => runTest('<xx / ><p>Hold the line.'))
	...
	it('attributes (no white space, no value, no quotes)', () =>
		runTest('<button class="test0"title="test1" disabled value=test2>adsf</button>'))
	...
	it('Long comment ending', () => runTest("<meta id='before'><!-- text ---><meta id='after'>"))
	...
	it('Implicit open p and br tags', () => runTest('<div>Hallo</p>World</br></ignore></div></p></br>'))

	it('lt followed by whitespace', () => runTest('a < b'))

	it('double attribute', () => runTest('<h1 class=test class=boo></h1>'))

	it('numeric entities', () => runTest('&#x61;&#x62&#99;&#100&#x66g&#x;&#x68'))

	it('legacy entities', () => runTest('&AMPel&iacutee&ampeer;s&lter&sum'))

	it('named entities', () => runTest('&amp;el&lt;er&CounterClockwiseContourIntegral;foo&bar'))
	...
	it('entity in attribute', () =>
		runTest("<a href='http://example.com/p&#x61;#x61ge?param=value&param2&param3=&lt;val&; & &'>"))

	it('double brackets', () => runTest('<<princess-purpose>>testing</princess-purpose>'))
	...
	it('Entities in attributes', () => runTest('<foo bar=&amp; baz="&amp;" boo=\'&amp;\' noo=>'))
	...
	it('CDATA more edge-cases', () => runTest('<![CDATA[foo]bar]>baz]]>', { recognizeCDATA: true }))
	...
	it('entity in attribute (#276)', () =>
		runTest('<img src="?&image_uri=1&&image;=2&image=3"/>?&image_uri=1&&image;=2&image=3'))
	...
	it('</title> in <script> (#745)', () => runTest("<script>'</title>'</script>"))
	...
	it('Self-closing indices (#941)', () => runTest('<xml><a/><b/></xml>', { xmlMode: true }))

	it('Entity after <', () => runTest('<&amp;'))

	it('Attribute in XML (see #1350)', () =>
		runTest('<Page\n    title="Hello world"\n    actionBarVisible="false"/>', { xmlMode: true }))
})
